  ...
  const bookingData = (e) => {
      ...
      repriceRequestList: [
        {
          ...
          brandedFareRefs:
            comboFare?.item[0]?.brandedFares !== null
              ? comboFare?.item[0]?.brandedFares[comboFare?.departureInx]?.ref
              : "",
        },
        {
          ...
          brandedFareRefs:
            comboFare?.item[1]?.brandedFares !== null
              ? comboFare?.item[1]?.brandedFares[comboFare?.returnIdx]?.ref
              : "",
        },
      ],
    };
    ...
    if (Object.keys(direction0).length > 0) {
      direction0.segments.map((i) =>
        priceCheck?.repriceRequestList[0]?.segmentCodeRefs.push(
          i.segmentCodeRef
        )
      );
    }
    if (Object.keys(direction1).length > 0) {
      direction1.segments.map((i) =>
        priceCheck?.repriceRequestList[1]?.segmentCodeRefs.push(
          i.segmentCodeRef
        )
      );
    }
    ...
    async function fetchOptions() {
      await requestComboPrice(priceCheck, {
        timeout: 600000,
      })
        .then((response) => {
          if (response.data !== null && response.data !== "") {
            if (Array.isArray(response.data)) {
              if (
                ...
                response.data[1].item2?.isSuccess
              ) {
                if (
                  response.data[0].item1?.isPriceChanged === false &&
                  response.data[0].item1?.isPriceChanged !== undefined
                ) {
                  if (
                    hasExtraService &&
    ...
    async function booking(data) {
      sendObj.comboBookRequests = [
        {
          uniqueTransID: data[0]?.item1.uniqueTransID,
          itemCodeRef: data[0]?.item1.itemCodeRef,
          priceCodeRef: data[0]?.item1.priceCodeRef,
        },
        {
          uniqueTransID: data[1]?.item1.uniqueTransID,
          itemCodeRef: data[1]?.item1.itemCodeRef,
          priceCodeRef: data[1]?.item1.priceCodeRef,
        },
      ];
      await requestComboBook(sendObj, {
        timeout: 600000,
      })
        .then((response) => {
          if (response.data?.length !== 0) {
            if (response.data[0].item2?.isSuccess === true) {
              setBookData(response);
    ...
    async function bookingNormal(price, uniqueTransID, itemCodeRef) {
                          ...
                          <div className="col-lg-4 col-sm-12">
                            ...
                            <div className="input-group mb-3">
                              ...
                              {moment(p?.passportExDate).isBefore(
                                ...
                                <Text color="red" pl="4px">
                                  Your passport expiry date is less than 6
                                  months{" "}
                                </Text>
                              )}
                ...
                <div className="col-lg-12">
                  <div class="form-check">
                    ...
                    >
                      ...
                      <span onClick={onOpen4} role="button">
                        <u style={{ color: "#7c04c0" }} className="fw-bold">
                          Booking Policy
                        </u>
                      </span>
                      <span className="mx-1">and</span>
                      <span onClick={onOpen5} role="button">
                        <u style={{ color: "#7c04c0" }} className="fw-bold">
                          Terms & Conditions
                        </u>
                      </span>
